package seminar02.shoppingCart;
...
*/
    ...
    void priceCartIsCorrectCalculated() {
        ...
        cart.addProductToCartByID(10);// 170 + 30 + 100
        // Assert
        assertEquals(300, cart.getTotalPrice(), "Стоимость корзины считается не корректно");
    }
    ...
    void priceCartProductsSameTypeIsCorrectCalculated() {
        ...
        cart.addProductToCartByID(1);
        cart.addProductToCartByID(1);
        cart.addProductToCartByID(1);// 170 + 30 + 100

        // Assert
        assertEquals(510, cart.getTotalPrice(), "Стоимость корзины считается не корректно");
    }
    ...
    void whenChangingCartCostRecalculationIsCalled() {
        ...
        assertEquals(270, cart.getTotalPrice(), "Стоимость корзины считается не корректно");
    }
    ...
    void quantityProductsStoreChanging() {
        int initialStock = shop.getProductsShop().get(0).getQuantity();
        cart.addProductToCartByID(1);
        int finalStock = shop.getProductsShop().get(0).getQuantity();
        assertEquals(1, initialStock - finalStock, "Остаток в магазине считается не корректно");
    }

    /**
     * 2.5. Создайте модульный тест для проверки, что если пользователь забирает все имеющиеся продукты определенного
     * типа из магазина, эти продукты больше не доступны для заказа.
     * <br><b>Ожидаемый результат:</b>
     * Больше такой продукт заказать нельзя, он не появляется на полке
     */
    @Test
    void lastProductsDisappearFromStore() {
        int initialQuantity = shop.getProductsShop().get(0).getQuantity();
        int overQuantity = initialQuantity + 1;
        for (int i = 0; i < overQuantity; i++) {
            cart.addProductToCartByID(1);
        }
        int cartProductQuantity = cart.cartItems.get(0).getQuantity();

        assertEquals(initialQuantity, cartProductQuantity, "Можно заказать больше имеющегося в магазине");
    }
    ...
    void deletedProductIsReturnedToShop() {
        ...
        int finalQuantity = shop.getProductsShop().get(0).getQuantity();

        assertEquals(1, finalQuantity - initialQuantity, "Некорректно считается возврат товара на полку");
    ...
    void incorrectProductRemoveCausesException() {
        int id = 1;
        assertThatThrownBy(() -> cart.removeProductByID(id)).isInstanceOf(RuntimeException.class)
                .hasMessage("В корзине не найден продукт с id: " + id);
    }
